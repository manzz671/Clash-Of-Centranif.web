...
async function getQuotes() {
  ...
  if (res.length > 0) {
    ...
    function acak() {
    let teracak = hasilNow;
      while (teracak === hasilNow) {
        console.log(teracak)
        teracak = Math.floor(Math.random() * res.length)
      }
      hasilNow = teracak
      return teracak;
    }
...
async function updateQuotesBox() {
  let quotes = await getQuotes()
  let qBox = document.getElementById("quotes")
  qBox.classList.add("fade-out")
  setTimeout(() => {
    qBox.innerText = `${quotes.isi}\n\n${quotes.by}`
